...
cp /mnt/hgfs/Macintosh\ HD/Users/flatballflyer/Desktop/sitgen.tar .
...
yum install -y php54 php54-mysql
...
######################################################################
# Install mySql jdbc drivers for Tomcat
cd /tmp
...
# This is also a good time for a VMWare snapshot
...
#########################################
# Configure Environment Vars - Add these values to
vi /etc/init.d/httpd
...
#########################################
# Configure JNDI Data Sources - add these resource entries to the contex in 
...
# Edit the following files to change the value of dragonFlyUrl (first line of <head>)
...
# Good time for a vmware snapshot

#########################################
# Install Situation Generator Prototype
cd /var/www/html
mkdir -p sitgen
cd sitgen
...
mysql -u root -p < loadAttributes.sql
mysql -u root -p < loadTemplates.sql
